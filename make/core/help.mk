...
CY_HELP_build_proj=Build a single project.
CY_HELP_build_proj_VERBOSE=Build a single target in the application. In the single-core applications, this target is the same as the "build" target.
...
CY_HELP_qbuild_proj=Builds a single project using the previous build's source list.\
...
CY_HELP_program_proj=Builds project's artifact file. In the single-core applications, this target is the same as the "program" target.
...
CY_HELP_qprogram_proj=Programs a built project to the target device without rebuilding.\
