...
ifneq ($(filter uvision5 uvision,$(MAKECMDGOALS)),)
_MTB_CORE__IDE_EXPORT_FINAL_MSG:=For more information, refer to the Keil uVision for ModusToolbox user guide.
else ifneq ($(filter ewarm8 ewarm,$(MAKECMDGOALS)),)
_MTB_CORE__IDE_EXPORT_FINAL_MSG:=For more information, refer to the IAR Embedded Workbench for ModusToolbox user guide.
endif
...
# mtbideexport stores template replace files as a map from source to destination.
...
ifneq ($(filter SECURE,$(VCORE_ATTRS)),)
	$(MTB__NOISE)echo;\
	echo "The project is an ARM Trustzone secure project. The following need to be enabled in Embedded Workbench IDE.";\
	echo "    Project->Options->General Options->32 bit->TrustZone";\
	echo "    Project->Options->Linker->Output->TrustZone import library"
endif
ifneq ($(filter NON_SECURE,$(VCORE_ATTRS)),)
	$(MTB__NOISE)echo;\
	echo "The project is an ARM Trustzone non-secure project. The following need to be enabled in Embedded Workbench IDE.";\
	echo "    Project->Options->General Options->32 bit->TrustZone";\
