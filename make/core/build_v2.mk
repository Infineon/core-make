...
_MTB_CORE__BUILD_ALL_ASFLAGS_UC=$(filter-out $(filter-out $(ASFLAGS),$(DISABLE_ASFLAGS)),$(MTB_RECIPE__ASFLAGS_UC) $(MTB_RECIPE__DEFINES))
else
...
_MTB_CORE__BUILD_ALL_ASFLAGS_LC=$(filter-out $(filter-out $(ASFLAGS),$(DISABLE_ASFLAGS)),$(MTB_RECIPE__ASFLAGS_LC))
else
...
ifneq ($(MTB_RECIPE__INCRSPFILE_ASM_UC),)
_MTB_CORE__BUILD_INCRSPFILE_ASM_UC:=$(MTB_RECIPE__INCRSPFILE_ASM_UC)
else
_MTB_CORE__BUILD_INCRSPFILE_ASM_UC:=$(MTB_RECIPE__INCRSPFILE_ASM)
endif
ifneq ($(MTB_RECIPE__INCRSPFILE_ASM_LC),)
_MTB_CORE__BUILD_INCRSPFILE_ASM_LC:=$(MTB_RECIPE__INCRSPFILE_ASM_LC)
else
_MTB_CORE__BUILD_INCRSPFILE_ASM_LC:=$(MTB_RECIPE__INCRSPFILE_ASM)
endif

# include 'lower case' assember if one is defined, otherwise whatever is defined
# if a lower/upper case assembler aren't defined, use the general version
ifeq ($(AS_LC),)
AS_LC:=$(AS)
endif
ifeq ($(AS_UC),)
...
# Compiler arguments
#
_MTB_CORE__BUILD_COMPILE_AS_UC=$(AS_UC) $(_MTB_CORE__BUILD_ALL_ASFLAGS_UC) $(_MTB_CORE__BUILD_INCRSPFILE_ASM_UC)$(MTB_TOOLS__OUTPUT_CONFIG_DIR)/inclist.rsp $(MTB_RECIPE__OUTPUT_OPTION)
_MTB_CORE__BUILD_COMPILE_AS_LC=$(AS_LC) $(_MTB_CORE__BUILD_ALL_ASFLAGS_LC) $(_MTB_CORE__BUILD_INCRSPFILE_ASM_LC)$(MTB_TOOLS__OUTPUT_CONFIG_DIR)/inclist.rsp $(MTB_RECIPE__OUTPUT_OPTION)
...
_MTB_CORE__CDB_BUILD_COMPILE_AS_UC=$(AS_UC) $(_MTB_CORE__BUILD_ALL_ASFLAGS_UC) $(MTB_RECIPE__INCLUDES) $(MTB_RECIPE__OUTPUT_OPTION)
_MTB_CORE__CDB_BUILD_COMPILE_AS_LC=$(AS_LC) $(_MTB_CORE__BUILD_ALL_ASFLAGS_LC) $(MTB_RECIPE__INCLUDES) $(MTB_RECIPE__OUTPUT_OPTION)
